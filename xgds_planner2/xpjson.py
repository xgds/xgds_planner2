...
JSON_SCHEMA_PATH_PLAN_SCHEMA = os.path.join(
    THIS_DIR, "xpjsonSpec", "xpjsonPlanSchemaDocumentSchema.json"
)

EXAMPLE_PLAN_SCHEMA_PATH = os.path.join(
    THIS_DIR, "xpjsonSpec", "examplePlanSchema.json"
)
EXAMPLE_PLAN_PATH = os.path.join(THIS_DIR, "xpjsonSpec", "examplePlan.json")
EXAMPLE_PLAN_LIBRARY_PATH = os.path.join(
    THIS_DIR, "xpjsonSpec", "examplePlanLibrary.json"
)
...
def resolveInheritanceLookup(
    spec, parentSpecLookup, inheritFields=(), localOnlyFields=()
):
    if "parent" in spec:
...
def resolveSchemaInheritance(schemaDict):
    ...
    commandSpecsLookup = resolveSpecInheritance(
        rawCommandSpecs,
    ...
    for c in commandSpecs:
        c.params = [
            resolveInheritanceLookup(p, paramSpecLookup) for p in c.get("params", [])
        ]

    for f in ("planParams", "stationParams", "segmentParams", "targetParams"):
        schemaDict[f] = [
            resolveInheritanceLookup(p, paramSpecLookup) for p in schemaDict.get(f, [])
        ]
...
class TypedObject(object):
    ...
    def __init__(self, objDict, schema=None, schemaParams=None, parseOpts=None):
    ...
    def get(self, fieldName, defaultVal="__unspecified__"):
        ...
        if schemaParam is not None and schemaParam.default is not None:
    ...
    def checkFields(self):
        ...
        for fieldName, spec in self.fields.iteritems():
            ...
            if val is None:
                ...
                val = spec.default
            if val is not None:
                assert isValueOfType(
                    val, spec.valueType
                ), "%s should have valueType %s in %s" % (
                    fieldName,
                    spec.valueType,
                    self._objDict,
                )
            if val is not None and spec.validMethod is not None:
                ...
                assert validMethod(val), "%s should satisfy %s in %s" % (
                    fieldName,
                    validMethod,
                    self._objDict,
                )
...
class ParamSpec(TypedObject):
    ...
    def isValidValueType(self, val):
        return val in VALUE_TYPE_CHOICES or parseArrayType(val)
    ...
    def isChoicesValid(self, enum):
        for val, desc in enum:
            if not isValueOfType(val, self.get("valueType")) or not isinstance(
                desc, (str, unicode)
            ):
                return False
        return True
        ...
        if self.minimum is not None:
            if self.strictMinimum:
                if not val > self.minimum:
                    return "value %s should be strictly greater than minimum %s" % (
                        val,
                        repr(self.minimum),
                    )
            else:
                if not val >= self.minimum:
                    return "value %s should be greater than or equal to minimum %s" % (
                        val,
                        repr(self.minimum),
                    )
        ...
        if self.maximum is not None:
            if self.strictMaximum:
                if not val < self.maximum:
                    return "value %s should be strictly less than maximum %s" % (
                        val,
                        repr(self.maximum),
                    )
            else:
                if not val <= self.maximum:
                    return "value %s should be less than or equal to maximum %s" % (
                        val,
                        repr(self.maximum),
                    )
...
class PlanSchema(Document):
    ...
    def __init__(self, objDict, **kwargs):
        ...
        for fieldName in paramsFields:
            setattr(self, fieldName + "Lookup", getIdDict(self.get(fieldName)))
...
class PathElement(TypedObject):
    ...
    def __init__(self, objDict, **kwargs):
        ...
        self.sequence = [Command(elt, **kwargs) for elt in self.sequence]
...
class Command(TypedObject):
    ...
    def __init__(self, objDict, **kwargs):
        kwargs["schemaParams"] = (
            kwargs["schema"].commandSpecsLookup[objDict["type"]].paramsLookup
        )
...
def loadDocumentFromDict(docDict, schema=None, parseOpts=None):
    ...
    return transformBottomUp(
        docDict, decodeWithClassName, schema=schema, parseOpts=parseOpts
    )
...
def getCrsTransformRoversw(crs):
    ...
    proj = pyproj.Proj(
        "+proj=utm +ellps=WGS84 +zone=%(zone)s +north" % crs["properties"]
    )
