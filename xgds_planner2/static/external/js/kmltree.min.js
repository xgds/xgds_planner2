var kmltreeManager=function(){function p(l){i=l;google.earth.addEventListener(i,"balloonopening",j);google.earth.addEventListener(i,"balloonclose",o);google.earth.addEventListener(i.getGlobe(),"click",function(k){if(k.getButton()!==-1)if(k.getTarget().getType()==="GEGlobe"&&$(".kmltree-selected").length)for(k=0;k<f.length;k++){var m=$(f[k].api.opts.element);m.find(".kmltree-selected").length+m.find(".kmltree-breadcrumb").length>0&&f[k].instance.clearSelection()}})}that={};var f=[],i,e={};that.register=
function(l,k){f.length===0&&p(k.opts.gex.pluginInstance);f.push({key:"kmltree-tree-"+f.length.toString(),instance:l,api:k})};that.remove=function(l){for(var k=0;k<f.length;k++)if(f[k].instance===l){f.splice(k,1);break}for(var m in e)e[m].instance===l&&delete e[m];return l};that.pauseListeners=function(l){google.earth.removeEventListener(i,"balloonopening",j);google.earth.removeEventListener(i,"balloonclose",o);l();google.earth.addEventListener(i,"balloonopening",j);google.earth.addEventListener(i,
"balloonclose",o)};var c=function(l){for(var k=0;k<f.length;k++)if(f[k].instance===l)return f[k].api},j=function(l){var k=l.getFeature(),m=g(k);if(m){l.preventDefault();i.setBalloon(null);l=false;var C=k.getId();if(C){l=m.api.opts.selectable;if(typeof l==="function")l=l(k)}l&&m.instance.selectById(C,k);v(k,m);return false}},o=function(){$("#kmltree-balloon-iframe").remove();for(var l=0;l<f.length;l++){var k=$(f[l].api.opts.element);k.find(".kmltree-selected").length+k.find(".KmlNetworkLink.kmltree-breadcrumb:not(.loaded)").length===
1&&k.find(".kmltree-cursor-2").length===0&&f[l].instance.clearSelection()}};that._clearEverythingButMe=function(l){for(var k=0;k<f.length;k++)if(f[k].instance!==l)if($(f[k].api.opts.element).find(".kmltree-selected").length||$(f[k].api.opts.element).find(".kmltree-breadcrumb").length)f[k].instance.clearSelection()};var r=function(l,k){if(l.getUrl()===k)return true;if(l.getElementByUrl(k))return true},g=function(l){l=l.getUrl();var k=l.split("#")[0];if(e[k])return e[k];for(var m=0;m<f.length;m++)if(r(f[m].instance.kmlObject,
l)){e[k]=f[m];return f[m]}for(m=0;m<f.length;m++)for(var C=f[m].api.docs,u=0;u<C.length;u++)if(r(C[u],l)){e[k]=f[m];return f[m]}return false};that.getOwner=function(l){return(l=g(l))?l.instance:false};var v=function(l,k){$(window).unbind("message.kmlTreeIframeEvents");var m;k=k.instance?k.instance:k;var C=k.api?k.api:c(k);m=C.opts.displayEnhancedContent;if(typeof m==="function")m=m(l);if(m){var u=l.getBalloonHtmlUnsafe(),w=l.getBalloonHtml();w=$.trim(w.replace(/\s*<\!--\s*Content-type: mhtml-die-die-die\s*--\>/,
""));w=w!=$.trim(u)}if(m&&w){m=i.createHtmlDivBalloon("");var x=document.createElement("iframe");x.setAttribute("src",C.opts.iframeSandbox);x.setAttribute("frameBorder","0");x.setAttribute("id","kmltree-balloon-iframe");w=document.createElement("div");$(w).append(x);$(x).one("load",function(){$(window).bind("message.kmlTreeIframeEvents",{window:x.contentWindow},function(y){var t=y.originalEvent;if(t.source===y.data.window){y=i.getBalloon();var H=y.getFeature();if(!(!$("#kmltree-balloon-iframe").length||
!(t.data.match(/width/)||t.data.match(/unknownIframeDimensions/))||y.getType()!=="GEHtmlDivBalloon")){var K=g(H);t=JSON.parse(t.data);if(t.unknownIframeDimensions){t=K.api.opts.unknownIframeDimensionsDefault;if(typeof t==="function")t=t(H)}var L=$("#kmltree-balloon-iframe");y.setMinWidth(t.width);y.setMaxWidth(t.width+t.width*0.1);y.setMinHeight(t.height);y.setMaxHeight(t.height+t.height*0.1);L.height(t.height);L.width(t.width);$(K.instance).trigger("balloonopen",[y,H])}}});this.contentWindow.postMessage(JSON.stringify({html:Base64.encode(u),
callback:Base64.encode(C.opts.sandboxedBalloonCallback.toString())}),"*")});m.setContentDiv(w)}else{m=i.createFeatureBalloon("");var P=function(y){google.earth.removeEventListener(i,"balloonopening",P);setTimeout(function(){$(k).trigger("balloonopen",[y.getBalloon(),y.getFeature()])},1)};google.earth.addEventListener(i,"balloonopening",P)}m.setFeature(l);i.setBalloon(m)};that._openBalloon=v;return that}(),Base64={_keyStr:"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=",encode:function(p){var f=
"",i,e,c,j,o,r,g=0;for(p=Base64._utf8_encode(p);g<p.length;){i=p.charCodeAt(g++);e=p.charCodeAt(g++);c=p.charCodeAt(g++);j=i>>2;i=(i&3)<<4|e>>4;o=(e&15)<<2|c>>6;r=c&63;if(isNaN(e))o=r=64;else if(isNaN(c))r=64;f=f+this._keyStr.charAt(j)+this._keyStr.charAt(i)+this._keyStr.charAt(o)+this._keyStr.charAt(r)}return f},decode:function(p){var f="",i,e,c,j,o,r=0;for(p=p.replace(/[^A-Za-z0-9\+\/\=]/g,"");r<p.length;){i=this._keyStr.indexOf(p.charAt(r++));e=this._keyStr.indexOf(p.charAt(r++));j=this._keyStr.indexOf(p.charAt(r++));
o=this._keyStr.indexOf(p.charAt(r++));i=i<<2|e>>4;e=(e&15)<<4|j>>2;c=(j&3)<<6|o;f+=String.fromCharCode(i);if(j!=64)f+=String.fromCharCode(e);if(o!=64)f+=String.fromCharCode(c)}return f=Base64._utf8_decode(f)},_utf8_encode:function(p){p=p.replace(/\r\n/g,"\n");for(var f="",i=0;i<p.length;i++){var e=p.charCodeAt(i);if(e<128)f+=String.fromCharCode(e);else{if(e>127&&e<2048)f+=String.fromCharCode(e>>6|192);else{f+=String.fromCharCode(e>>12|224);f+=String.fromCharCode(e>>6&63|128)}f+=String.fromCharCode(e&
63|128)}}return f},_utf8_decode:function(p){for(var f="",i=0,e=c1=c2=0;i<p.length;){e=p.charCodeAt(i);if(e<128){f+=String.fromCharCode(e);i++}else if(e>191&&e<224){c2=p.charCodeAt(i+1);f+=String.fromCharCode((e&31)<<6|c2&63);i+=2}else{c2=p.charCodeAt(i+1);c3=p.charCodeAt(i+2);f+=String.fromCharCode((e&15)<<12|(c2&63)<<6|c3&63);i+=3}}return f}};
(function(){var p={};this.tmpl=function f(i,e){var c=!/\W/.test(i)?p[i]=p[i]||f(document.getElementById(i).innerHTML):new Function("obj","var p=[],print=function(){p.push.apply(p,arguments);};with(obj){p.push('"+i.replace(/[\r\t\n]/g," ").split("<%").join("\t").replace(/((^|%>)[^\t]*)'/g,"$1\r").replace(/\t=(.*?)%>/g,"',$1,'").split("\t").join("');").split("%>").join("p.push('").split("\r").join("\\'")+"');}return p.join('');");return e?c(e):c}})();
...
(function(){function p(e,c){var j=/^(.*)\//;return e.authority&&!e.path?"/"+c:e.getPath().match(j)[0]+c}function f(e){if(!e)return"";e=e.replace(/\/\.\//g,"/");for(e=e.replace(/\/\.$/,"/");e.match(i);)e=e.replace(i,"/");for(e=e.replace(/\/([^\/]*)\/\.\.$/,"/");e.match(/\/\.\.\//);)e=e.replace(/\/\.\.\//,"/");return e}var i=/\/((?!\.\.\/)[^\/]*)\/\.\.\//;URI=function(e){e||(e="");e=e.match(/^(?:([^:\/?\#]+):)?(?:\/\/([^\/?\#]*))?([^?\#]*)(?:\?([^\#]*))?(?:\#(.*))?/);var c=e[1]||null,j=e[2]||null,o=
e[3]||null,r=e[4]||null,g=e[5]||null;this.getScheme=function(){return c};this.setScheme=function(v){c=v};this.getAuthority=function(){return j};this.setAuthority=function(v){j=v};this.getPath=function(){return o};this.setPath=function(v){o=v};this.getQuery=function(){return r};this.setQuery=function(v){r=v};this.getFragment=function(){return g};this.setFragment=function(v){g=v}};URI.prototype.toString=function(){var e="";if(this.getScheme())e+=this.getScheme()+":";if(this.getAuthority())e+="//"+this.getAuthority();
...
c){var j=new URIQuery;if(c)j.separator=c;j.addStringParams(e);return j};URIQuery.prototype.addStringParams=function(e){e=e.split(this.separator);for(var c,j,o=0;o<e.length;o++){c=e[o].split("=",2);j=c[0].replace(/\+/g," ");c=c[1].replace(/\+/g," ");this.params.hasOwnProperty(j)||(this.params[j]=[]);this.params[j].push(c)}};URIQuery.prototype.getParam=function(e){if(this.params.hasOwnProperty(e))return this.params[e][0];return null};URIQuery.prototype.toString=function(){var e=[],c;c=this.params;var j=
[],o;for(o in c)c.hasOwnProperty(o)&&j.push(o);c=j.sort();for(j=0;j<c.length;j++)for(o=0;o<this.params[c[j]].length;o++)e.push(c[j].replace(/ /g,"+")+"="+this.params[c[j]][o].replace(/ /g,"+"));return e.join(this.separator)}})();
var kmltree=function(){function p(c){var j=document.createElement("a");j.href=c;return j.href}var f=function(c){if(c.success&&c.error&&c.tree){this.queue=[];this.opts=c}else throw"missing required option";};f.prototype.add=function(c,j){this.queue.push({node:c,callback:j,loaded:false,errors:false})};f.prototype.execute=function(){if(this.queue.length===0)this.opts.success([]);else for(var c=0;c<this.queue.length;c++){var j=this.queue[c];j.node.data("queueItem",j);if(!j.loaded&&!j.loading){var o=this;
$(j.node).bind("loaded",function(r,g,v){o.nodeLoadedCallback(r,g,v)});this.opts.tree.openNetworkLink(j.node);j.loading=true}}};f.prototype.nodeLoadedCallback=function(c,j){var o=j.data("queueItem");if(o.loaded===true)throw"event listener fired twice for "+j.find(">span.name").text();o.loaded=true;o.loading=false;$(j).unbind("loaded");o.callback(j);this.execute();this.finish(o)};f.prototype.finish=function(){for(var c=true,j=true,o=0;o<this.queue.length;o++){c=c&&this.queue[o].loaded;j=j&&!this.queue[o].errors}if(c){j?
...
g.hideLoading=U;var da=function(a,b,d){u={};if(a){v=0;g.kmlObject=a;k.push(a);g.kmlObject.setVisibility(true);var h=y(a,d),n;if(c.displayDocumentRoot===false)n=h.children[0].children;else if(c.displayDocumentRoot===true)n=h.children[0];else{n=h.children[0].children;for(var q=0,z=false;q<n.length&&z===false;){z=n[q].type==="KmlPlacemark";q++}n=z?h.children[0]:h.children[0].children}n=S(n);c.element.find("div.kmltree").remove();c.element.find(".kmltree-loading").before(['<div UNSELECTABLE="on" class="kmltree"><h4 UNSELECTABLE="on" class="kmltree-title">',
...
g.kmlObject.getParentNode()&&c.gex.dom.removeObject(g.kmlObject);g.kmlObject=null;k=[]},pa=function(){var a=localStorage.getItem("kmltree-("+c.url+")");return a?JSON.parse(a):false};g.destroy=function(){C=true;kmltreeManager.remove(g);if(c.restoreState&&window.localStorage){var a=JSON.stringify(W());localStorage.setItem("kmltree-("+c.url+")",a)}ea();l=null;l={};if(a=m.getBalloon())if(a=a.getFeature())(a=kmltreeManager.getOwner(a))&&a.instance===g&&m.setBalloon(null);m.setBalloon(null);a=c.element.attr("id");
...
a.append("<ul>"+A+"</ul>");a.addClass("open");F(a,"open",a.hasClass("open"));a.removeClass("loading");a.addClass("loaded");l[a.attr("id")]=s;k.push(s);$(a).attr("data-rememberedLink",V.length.toString());V.push(b);$(a).trigger("loaded",[a,s]);$(g).trigger("networklinkload",[a,s])}else{alert("Error loading "+d);a.addClass("error");$(g).trigger("kmlLoadError",[s]);a.removeClass("open")}})}};var V=[];g.getNetworkLink=function(a){return(a=$(a).attr("data-rememberedLink"))&&V.length>=a?V[a]:false};var la=
