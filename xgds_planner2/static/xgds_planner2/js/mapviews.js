	...
	var PlanKmlView = Backbone.View
			.extend({
				...
				initialize : function() {
					...
					this.stationsFolder = ge.gex.dom.buildFolder({
						name : 'stations'
					});
					this.dragHandlesFolder = ge.gex.dom.buildFolder({
						name : 'dragHandles'
					});
					this.segmentsFolder = ge.gex.dom.buildFolder({
						name : 'segments'
					});
					...
					if (bbox != undefined) {
						...
						var folder = ge.gex.dom.addFolder([
								ge.gex.dom.buildPointPlacemark([ bbox[0],
										bbox[1] ]),
								ge.gex.dom.buildPointPlacemark([ bbox[2],
										bbox[3] ]) ]);
						var bounds = ge.gex.dom.computeBounds(folder);
						ge.gex.view.setToBoundsView(bounds, {
							aspectRatio : aspect,
							scaleRange : 1.2
						});
				...
				setMode : function(modeName) {
					...
					var mode = _.isObject(modeName) ? modeName
				...
				addStationsMouseDown : function(evt) {
					...
					if (!_.isUndefined(app.State.addStationLocation)
						...
						distance = Math.sqrt(Math.pow(evt.getClientX()
								- app.State.addStationLocation[0], 2), Math
								.pow(evt.getClientY()
										- app.State.addStationLocation[1], 2));
					}
				...
				addStationsMouseMove : function(evt) {
					if (_.isUndefined(app.State.mouseDownLocation))
						return;
					var distance = Math.sqrt(Math.pow(evt.getClientX()
							- app.State.mouseDownLocation[0], 2), Math.pow(evt
							.getClientY()
							- app.State.mouseDownLocation[1], 2));
				...
				drawMidpoints : function() {
					if (!this.midpointsFolder) {
						this.midpointsFolder = this.ge.gex.dom.buildFolder({
							name : 'midpoints'
						});
					}
			...
			.extend({
				...
				redraw : function() {
					...
					if (this.placemark.getGeometry().getLongitude() - coords[0] != 0
							|| this.placemark.getGeometry().getLatitude()
									- coords[1] != 0)
						this.redrawHandles();
				...
				buildStyle : function() {
					...
					var iconUrl = app.State.stationSelected === this.model
							...
							: (this.model.get('isDirectional') && app.options.directionalStations) ? 'http://earth.google.com/images/kml-icons/track-directional/track-0.png'
				...
				dragRotateHandleCoords : function() {
					...
					var theta = this.model.get('headingDegrees') * Math.PI
							/ 180.00; // radians
	...
	var SegmentLineView = Backbone.View
			.extend({
				...
				render : function() {
					...
					var style = this.model.get('sequence').isEmpty() ? '#segment'
							: '#segment_with_commands';
				...
				updateGeom : function(point1, point2) {
					...
					if (_.isUndefined(point2) && !_.isUndefined(point1)
							&& point1.cid) {
					...
					if (point1 && point2) {
						...
						_.each([ point1, point2 ], function(point) {
							if (_.isArray(point)) {
								coords.push(point);
							} else if (_.isObject(point) && _.has(point, 'lat')
									&& _.has(point, 'lng')) {
								coords.push([ point.lat, point.lng ]);
							} else if (_.isObject(point)
									&& _.isFunction(point.get)) {
